...
function getBoolValue(value) {
    if (isBool(value)) {
        ...
        throw new Error("Interpreter error: "  +
                        "The argument of getBoolValue " +
                        "is not a Bool value.");
    }
}
function createList(l) {
    return ["List",l.map(Number)];
}
...
function getEnvBindings (env) {
    if (isEnv(env)) {
        ...
        throw new Error("Interpreter error: "  +
                        "The argument of getEnvBindings " +
...
function initEnv() {
    return update(createEmptyEnv(), ["x","y"], 
                [ createNum(5), createNum(6) ]);
}
