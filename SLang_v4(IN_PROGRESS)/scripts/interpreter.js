...
function evalExp(exp,envir) {
    ...
    } else if (A.isSuperMethodCall(exp)) {
        args = evalExps(A.getSuperMethodCallArgs(exp),envir);
        return findAndInvokeMethod(A.getSuperMethodCallMethod(exp),
            E.getClassNameName(E.lookup(envir,"_super")),
            E.lookup(envir,"_this"),
            args
        );
    } else {
